...
const Button: React.FC<ButtonProps> = ({ src, children, active, soon }) => {
    return <button className={tw("flex items-center gap-2 relative", active && "font-bold", soon && "text-grey-70")}>
        <Image alt="" src={src} />
        <When condition={soon}>
...
const TopNavbar = () => {
    return <div className="rounded-2xl p-4 bg-white w-full flex items-center gap-12 justify-center text-grey-100 mb-8">
        <Button src={LogoPlane} active><span className="text-left">Private<br />Flight</span></Button>
